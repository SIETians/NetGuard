...
/** Convert raw OS string to friendly emoji + label */
function friendlyOS(osString) {
    ...
    if (lower.includes('centos') || lower.includes('rhel')) return '🐧 Enterprise Linux';
    if (lower.includes('bsd')) return '🐦 BSD';
    
    return `🔌 ${osString}`;
}

/** Build device card HTML with icon, vendor, IP, MAC */
function buildDeviceCard(device, index, cardId) {
    const statusBadge = device.status === 'self' ? '🏠' : (device.status === 'up' ? '✅' : '📋');
    const deviceIcon = device.icon || '🔌';
    const deviceLabel = device.device_label || 'Unknown Device';
    const deviceVendor = device.vendor || 'Unknown Vendor';
    
    return `
        <div class="device-card" id="${cardId}">
            <div style="display: flex; gap: 12px; margin-bottom: 10px;">
                <div class="device-icon-col" title="${deviceLabel}">${deviceIcon}</div>
                <div class="device-info-col">
                    <div class="device-label">${deviceLabel}</div>
                    <div class="device-vendor">${deviceVendor}</div>
                </div>
                <div style="margin-left: auto; font-size: 1.3rem;">${statusBadge}</div>
            </div>
...
radarBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        devices.forEach((device, index) => {
            ...
            const cardHTML = buildDeviceCard(device, index, cardId);
    ...
    try {
        ...
        if(isHostMachine) {
            ...
            let hostHTML = `
                ...
                <div style="margin-bottom: 8px;"><strong>True OS:</strong> <span style="font-family: 'JetBrains Mono', monospace;">${friendlyOS(data.os_detected || 'Unknown')}</span></div>
                <div style="margin-bottom: 8px;"><strong>Inferred OS:</strong> <span style="font-family: 'JetBrains Mono', monospace;">${friendlyOS(data.os_inferred || 'Unknown')}</span></div>
                <div style="margin-bottom: 8px;"><strong>Device Type:</strong> <span>${data.device_icon || '🔌'} ${data.device_label || 'Unknown Device'}</span></div>
                <div style="margin-bottom: 8px;"><strong>NIC Vendor:</strong> <span style="font-family: 'JetBrains Mono', monospace; font-size: 0.9rem;">${data.mac_vendor || 'Unknown Vendor'}</span></div>
            `;
        ...
        if(data.results.length === 0) {
            resultsContainer.innerHTML = `<span class="port-badge badge-INFO">Device Secured - No Vulnerable Ports Detected</span>`;
            return;
        }
        ...
        data.results.forEach(portData => {
            if(portData.risk === 'CRITICAL' || portData.risk === 'WARNING') {
                ...
                hasCritical = true;
                if(portData.risk === 'CRITICAL') {
                    showThreatPopup(ip, portData.port);
                }
            } else {
                totalSafe++;
            }
...
function showThreatPopup(ip, port) {
    const container = document.getElementById('threat-popup-container');
    const notification = document.createElement('div');
    notification.className = 'toast-notification';
    notification.innerHTML = `🚨 CRITICAL THREAT DETECTED: Port ${port} on ${ip}`;
